...
import com.rincon.espacio.ui.components.Divider
 ...
 * Editor de nota.
 *
 * Está ordenado por frecuencia de uso: primero escribir, después el aspecto y
 * sólo al final la parte de planificación. Se puede guardar una nota escribiendo
 * y pulsando "Guardar": todo lo demás es opcional.
 */
@Composable
fun NoteEditorSheet(
    draft: NoteDraft?,
    ...
    CozySheet(visible = true, onDismiss = viewModel::dismissEditor) {
        Column(
            ...
            SheetTitle(
                title = if (draft.isNew) "Nueva nota" else "Editar nota",
                subtitle = if (draft.isNew) "Escribe y suéltala en tu escritorio." else null,
            )
            ...
            Spacer(Modifier.height(Space.l))
            Column(Modifier.padding(start = Space.xl)) { FieldLabel("Color del papel") }
            PaperColorPicker(
            ...
            Column(Modifier.padding(horizontal = Space.xl)) {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    Column(Modifier.weight(1f)) {
                        Text("Convertir en tarea", style = Rincon.type.bodyStrong, color = colors.textPrimary)
                        Text(
                            "Aparecerá en Hoy y podrás marcarla.",
                ...
                AnimatedVisibility(
                    ...
                    Column {
                        ...
                        Row(verticalAlignment = Alignment.CenterVertically) {
                            ...
                            Column(Modifier.weight(1f)) {
                                Text("Avisarme", style = Rincon.type.bodyStrong, color = colors.textPrimary)
                                Text(
                                    "Una notificación a la hora elegida.",
                                    style = Rincon.type.caption,
                                    color = colors.textSecondary,
                                )
                            }
                        ...
                        AnimatedVisibility(
                            ...
                            Column {
                                ...
                                Row(horizontalArrangement = Arrangement.spacedBy(Space.s)) {
                                    CozyChip(
                                        label = "Sonido",
                                        selected = draft.reminderSound,
                                        icon = RinconIcons.Speaker,
                                        onClick = {
                                            viewModel.editDraft { it.copy(reminderSound = !it.reminderSound) }
                                        },
                                        modifier = Modifier.weight(1f),
                                    )
                                    CozyChip(
                                        label = "Vibración",
                                        selected = draft.reminderVibrate,
                                        icon = RinconIcons.Vibrate,
                                        onClick = {
                                            viewModel.editDraft { it.copy(reminderVibrate = !it.reminderVibrate) }
                                        },
                                        modifier = Modifier.weight(1f),
                                    )
                                }
                ...
                draft.subtasks.forEachIndexed { index, subtask ->
                    Row(
                        ...
                        Box(
                            ...
                            RinconIcon(RinconIcons.Close, "Quitar subtarea", tint = colors.textMuted, size = 18.dp)
                        }
                    }
                }
                Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                if (subjects.isNotEmpty() || goals.isNotEmpty()) {
                    ...
                    Row(
                        ...
                        CozyChip(
                            ...
                            onClick = {
                                viewModel.editNote { it.copy(subjectId = null, goalId = null) }
                            },
                        )
